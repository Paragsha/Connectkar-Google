    ...
    companion object {
        ...
        val MIGRATION_9_10 = object : androidx.room.migration.Migration(9, 10) {
            override fun migrate(db: androidx.sqlite.db.SupportSQLiteDatabase) {
                ...
                """.trimIndent())
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_user_listing_interactions_listingId` ON `user_listing_interactions` (`listingId`)")
                db.execSQL("CREATE INDEX IF NOT EXISTS `index_user_listing_interactions_userId` ON `user_listing_interactions` (`userId`)")
                db.execSQL("""
                    INSERT OR IGNORE INTO `user_listing_interactions` (`userId`, `listingId`, `isLiked`, `isBookmarked`)
                    SELECT u.uid, l.id, l.isLikedByMe, l.isBookmarked
