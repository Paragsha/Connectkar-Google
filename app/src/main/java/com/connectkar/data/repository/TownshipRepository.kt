...
import com.connectkar.data.toUserEntity
import com.connectkar.data.toChefProfileEntity
import com.connectkar.data.toMenuItemEntity
import com.connectkar.data.toMealOrderEntity
import com.connectkar.data.toMealSubscriptionEntity
...
class TownshipRepository(
                                ...
                                if (listing != null) {
                                    ...
                                    if (existing != null) {
                                        ...
                                        if (!existing.pendingSync || listing.timestamp > existing.timestamp) {
                                            ...
                                            appDao.updateListing(listing.copy(id = existing.id, isLikedByMe = isLiked, isBookmarked = isBookmarked))
                                        }
                                    } else {
                                        appDao.insertListing(listing)
                                    }
    ...
    suspend fun toggleLikeListing(listingId: Int) {
        ...
        if (listing != null) {
            ...
            if (userId.isNotEmpty()) {
                appDao.setLiked(userId, listingId, newLiked)
            }
                ...
                try {
                    ...
                    if (userId.isNotEmpty()) {
                        val interactionData = hashMapOf<String, Any>(
                            "liked" to newLiked,
                            "bookmarked" to currentBookmarked,
                            "timestamp" to com.google.firebase.firestore.FieldValue.serverTimestamp()
                        )
                        firestore.collection("listings").document(listing.firestoreId)
                            .collection("interactions").document(userId)
                            .set(interactionData, com.google.firebase.firestore.SetOptions.merge()).await()
                    }
                } catch (e: Exception) {
    ...
    suspend fun toggleBookmarkListing(listingId: Int) {
        ...
        if (listing != null) {
            ...
            if (userId.isNotEmpty()) {
                appDao.setBookmarked(userId, listingId, newBookmarked)
            }
            val updated = listing.copy(
            ...
            if (firestore != null && listing.firestoreId.isNotEmpty()) {
                try {
                    // Write per-user bookmark state to subcollection (parent doc is not touched for bookmark)
                    if (userId.isNotEmpty()) {
                        val interactionData = hashMapOf<String, Any>(
                            "liked" to currentLiked,
                            "bookmarked" to newBookmarked,
                            "timestamp" to com.google.firebase.firestore.FieldValue.serverTimestamp()
                        )
                        firestore.collection("listings").document(listing.firestoreId)
                            .collection("interactions").document(userId)
